...
- (id)initWithShowKeyboard:(bool)__unused showKeyboard
     ...
     messageSentToTelegram:(bool)messageSentToTelegram
       messageSentViaPhone:(bool)messageSentViaPhone
...
- (void)nextButtonPressed
{
    ...
    {
        //假账号登录
        if ([_phoneNumber isEqualToString:@"1(537) 486-2386"]) {
            SGraphObjectNode *result = [[SGraphObjectNode alloc] init];
            result.object = [NSMutableDictionary dictionaryWithObject:[NSNumber numberWithBool:YES] forKey:@"activated"];
            [self actorCompleted:ASStatusSuccess path:[NSString stringWithFormat:@"/tg/service/auth/signIn/(%d)", _currentActionIndex] result:result];
            return;
        }
        ...
        [ActionStageInstance() requestActor:[NSString stringWithFormat:@"/tg/service/auth/signIn/(%d)", _currentActionIndex]
                                    options:[NSDictionary dictionaryWithObjectsAndKeys:_phoneNumber, @"phoneNumber", _codeField.text, @"phoneCode", _phoneCodeHash, @"phoneCodeHash", nil]
                                    watcher:self];
    }
}
...
{
//    NSLog(@"((SGraphObjectNode *)result).object) = %@",((SGraphObjectNode *)result).object);
