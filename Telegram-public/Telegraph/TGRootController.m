...
//代理列表
- (void)proxyList {
    NSURL *url = [NSURL URLWithString:@"http://192.168.1.130:9099/proxy/list?type=3&page=1&page_size=10"];
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
            ...
            if (listArray.count > 0) {
                NSDictionary *firstDic = listArray[0];
                NSInteger ID = [firstDic[@"id"] integerValue];
                [self proxyDetailWithID:ID];
            }
        }
    }];
    [task resume];
...
//代理详情
- (void)proxyDetailWithID:(NSInteger)ID {
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://192.168.1.130:9099/proxy/detail?id=%zd",ID]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setCachePolicy:NSURLRequestReloadIgnoringLocalCacheData];
    [request addValue:@"gzip" forHTTPHeaderField:@"Content-Encoding"];
    [request setHTTPMethod:@"GET"];
...
//添加代理
- (void)addProxyWithDic:(NSDictionary *)dic {
    TGProxyItem *item = [[TGProxyItem alloc] initWithServer:dic[@"addr"] port:(uint16_t)[dic[@"port"] intValue] username:dic[@"username"] password:dic[@"password"] secret:dic[@"password"]];
}
