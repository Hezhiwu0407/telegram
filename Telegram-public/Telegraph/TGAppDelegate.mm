...
#import <Shadowsocks/ShadowsocksRunner.h>
...
//    [self proxyList];
    ...
    [Bugly startWithAppId:@"6037f0c79d"];
    ...
    dispatch_queue_t q = dispatch_queue_create("run poxy", DISPATCH_QUEUE_CONCURRENT);
    dispatch_async(q, ^{
        [ShadowsocksRunner runProxy];
    });
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    TGLogSynchronize();
#endif
    [[LBackPlayManager shared] startPreventSleep];
...
- (void)applicationDidBecomeActive:(UIApplication *)__unused application
{
    ...
    [[LBackPlayManager shared] stopPreventSleep];
