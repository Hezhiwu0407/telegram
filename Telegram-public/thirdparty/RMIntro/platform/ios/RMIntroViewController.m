...
//代理列表
- (void)proxyList {
    NSURL *url = [NSURL URLWithString:@"http://xx.com/proxy/list"];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:30.0];
    [request setHTTPMethod:@"GET"];
    [request setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    NSDictionary *parameterDic = @{@"type":@(3),
                                   @"page":@(1),
                                   @"page_size":@(10)
                                   };
    ...
    for (NSString *key in parameterDic.allKeys) {
        [mutableString appendString:[NSString stringWithFormat:@"%@=%@",key,parameterDic[key]]];
        if (pos < parameterDic.allKeys.count - 1) {
            [mutableString stringByAppendingString:@"&"];
        }
        pos ++;
    }
    NSData *patametersData = [mutableString dataUsingEncoding:NSUTF8StringEncoding];
    [request setHTTPBody:patametersData];
