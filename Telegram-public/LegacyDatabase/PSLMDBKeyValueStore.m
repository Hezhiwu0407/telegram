...
- (bool)_open:(NSUInteger)size
{
    ...
    if (removedReaders != 0) {
//        NSLog(@"[PSLMDBKeyValueStore removed %d stale readers]", removedReaders);
    }
...
- (void)sync
{
    ...
    if (rc != MDB_SUCCESS) {
//        NSLog(@"[PSLMDBKeyValueStore sync: mdb_env_sync error %d]", rc);
    }
}
...
- (PSLMDBTable *)_createTableWithName:(NSString *)name
{
    ...
    if (result == nil)
    {
        ...
        if (rc != MDB_SUCCESS)
        {
//            NSLog(@"[PSLMDBKeyValueStore transaction begin failed %d]", rc);
            
            if (rc == MDB_PANIC)
            {
//                NSLog(@"[PSLMDBKeyValueStore critical error received]");
...
//            NSLog(@"[PSLMDBKeyValueStore mdb_dbi_open failed %d]", rc);
        }
        else
        {
            mdb_txn_commit(txn);
...
- (void)readInTransaction:(void (^)(id<PSKeyValueReader>))transaction
{
    ...
    if (table != nil)
    {
        ...
        if (rc != MDB_SUCCESS)
        {
//            NSLog(@"[PSLMDBKeyValueStore mdb_txn_begin failed %d", rc);
...
//                NSLog(@"[PSLMDBKeyValueStore critical error received]");
        ...
        else
        {
            ...
            if (rc != MDB_SUCCESS) {
//                NSLog(@"[PSLMDBKeyValueStore mdb_txn_commit error %d]", rc);
            }
        }
    }
}
